...
<div class="content-wrapper">
  ...
  <div class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1 class="m-0 text-dark">List of unit</h1>
        </div><!-- /.col -->
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="#">Home</a></li>
            <li class="breadcrumb-item active">List of unit</li>
          </ol>
        </div><!-- /.col -->
      </div><!-- /.row -->
    </div><!-- /.container-fluid -->
       ...
       <div class="col-12">
        <div class="card">
          <div class="card-header">
            ...
            <a style="float: right;color: #ffffff;background-color: #dc3545;" class="page-link" href="?f=unit_master_reg">Add New unit</a>
          </div>
          <!-- /.card-header -->
          <div class="card-body">
            ...
            <!--  === -->
               <table id="unit_list_table" class="table table-striped table-bordered" style="width:100%">
          <thead>
            <tr>
              <th>UnitID</th>
              <th>Unit Name</th>
...
<script type="text/javascript">
 ...
 // view_user();
    ...
    var table = $('#unit_list_table').DataTable( {
        "ajax": "php_controller/list_unit.php",
        "columnDefs": [ {
            "targets": -1,
            "data": null,
    ...
    $('#unit_list_table tbody').on( 'click', '#view_id', function () {
        var data = table.row( $(this).parents('tr') ).data();
        edit_me(data[0],'edit_units_modal','units','UnitID');
    } );

      $('#unit_list_table tbody').on( 'click', '#delete_id', function () {
        var data = table.row( $(this).parents('tr') ).data();
        delete_me(data[0],'units','UnitID','unit_list_table','load_orderStatus_table')
    } );
